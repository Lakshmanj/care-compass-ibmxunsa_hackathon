import { psychologistAgent, PsychologistResult } from './psychologistAgent';
...
export type DagOutput = {
    ...
    resources: Resource[];          // Hardcoded crisis. Present in flagged path only.
...
async function topicLeaf(topic: string, promptKey: string): Promise<DagOutput> {
    ...
    const internalResources: InternalResource[] = docs.map((d: any) => ({
        title: d.title,
        url: d.url,
